...
class SkeletonEventBinding : public SkeletonEventBindingBase
{
  public:
    ...
    virtual ResultBlank Send(const SampleType&, std::optional<SendTraceCallback>) noexcept = 0;
    ...
    /// \return On failure, returns an error code.
    virtual ResultBlank Send(SampleAllocateePtr<SampleType>, std::optional<SendTraceCallback>) noexcept = 0;
